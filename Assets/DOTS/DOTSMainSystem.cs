...
partial struct DOTSMainSystem : ISystem
{
    ...
    private struct Singleton : IComponentData
    {
        ...
        public void QueryCircleAll(ref CalcCollision action)
        {
        ...
        public void QueryCircleMap(float3 center, ref CalcCollision action)
        {
    ...
    {
        var parameter = SystemAPI.GetSingleton<DOTSSceneParameter>();
        ...
        switch (parameter.Mode)
        {
            case DOTSMode.AllBustCompiler:
                ...
                {
                    AllBustCompilerCollision(ref state);
                }
                break;
            ...
            case DOTSMode.AllBustCompilerJob:
                for (int iteration = 0; iteration < 1; ++iteration)
                {
                    AllBustCompilerJobCollision(ref state);
                }
                break;
    ...
    #region All_BustCompiler
    [BurstCompile]
    private void AllBustCompilerCollision(ref SystemState state)
    {
    ...
    #region All_BustCompiler_Job
    [BurstCompile]
    private void AllBustCompilerJobCollision(ref SystemState state)
    {
        m_Entities.Clear();
        m_Transforms.Clear();
        m_Map.Clear();
